...
def open_cliente():
    ...
    cursor = banco.cursor()
    try:
        cursor.execute("CREATE TABLE cliente (id_cliente integer PRIMARY KEY , nome text, cpf integer, "
                       "endereco text)")
    except:
        pass
    ...
    def deletar():
        try:
            for i in table.selection():
                ...
                cursor.execute("DELETE FROM cliente where id_cliente = ? and nome =? and cpf = ? and endereco =?",
                               valor)
                table.delete(i)
                banco.commit()
                clear()
        except sqlite3.IntegrityError:
            tkinter.messagebox.showwarning("Erro", "Esse registro não pode ser alterado")
            banco.commit()
    ...
    def entrada():
        ...
        if nome_textbox.get() and cpf_textbox.get() and ender_textbox.get():
            ls.append(input)
            for i in ls:
                cursor.execute("INSERT INTO cliente (nome,cpf,endereco) VALUES (?,?,?) ", i)
                cursor.execute("SELECT * FROM cliente where nome =? and cpf = ? and endereco =?", i)
    ...
    def confirmar():
        ...
        for i in table.selection():
            valor = table.item(i)['values']
            cursor.execute(
                "SELECT id_cliente FROM cliente where id_cliente = ? and nome =? and cpf = ? and endereco =?",
                valor)
            data = cursor.fetchall()
            for d in data:
                for m in d:
            ...
            table.delete(i)
            for n in ls:
                cursor.execute("SELECT * FROM cliente where nome =? and cpf = ? and endereco =?", n)
                data = cursor.fetchall()
                for d in data:
                    table.insert(parent='', index=0, values=d)
            banco.commit()
    ...
    def atualizar():
        ...
        if len(table.selection()) != 1:
            ...
            for i in table.selection():
                valor = table.item(i)['values']
                cursor.execute("SELECT * FROM cliente where id_cliente = ? and nome = ? and cpf = ? and endereco = ?",
                               valor)
                data = cursor.fetchall()
                for d in data:
                    for i in d:
    ...
    table.heading('id', text='Id_cliente')
    ...
    btn_deletar = tk.Button(frame, text="Deletar", width=8, command=deletar, font=("arial", 14), bg='#ef5350',
                            relief='raised', overrelief='ridge')
    btn_atualizar = tk.Button(frame, text="Atualizar", width=8, command=atualizar, font=("arial", 14), bg='#efca50',
                              relief='raised', overrelief='ridge')
    btn_confirmar = tk.Button(frame, text="Confirmar", width=8, command=confirmar, font=("arial", 14), bg='#41c48c',
                              relief='raised', overrelief='ridge')
    btn_enviar.place(relx=0.85, rely=0.05)
    btn_deletar.place(relx=0.85, rely=0.24)
    btn_atualizar.place(relx=0.85, rely=0.43)
